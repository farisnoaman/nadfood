...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen }) => {
  ...
  const filteredUserNotifications = notifications
    .filter((n: Notification) => {
      ...
      const isVisible = (roleMatch || userMatch) && categoryMatch;

      if (!isVisible && (filter === 'all' || n.category === filter)) {
        logger.debug(`Notification filtered out: ${n.message}`, {
          roleMatch, userMatch, categoryMatch,
          targetRoles: n.targetRoles, userRole: currentUser.role,
          targetUserIds: n.targetUserIds, userId: currentUser.id
        });
      }
          ...
          {filters.map(f => {
            ...
            return (
              <button
                ...
                className={`px-2 py-1 text-xs rounded-full transition-colors ${isActive
        ...
        {filteredUserNotifications.length > 0 ? (
          filteredUserNotifications.map(notification => (
            <div
              ...
              className={`p-3 border-b dark:border-secondary-700 last:border-b-0 cursor-pointer hover:bg-secondary-50 dark:hover:bg-secondary-700/50 ${!notification.read ? 'bg-primary-50 dark:bg-primary-900/30' : ''
                }`}
            >
