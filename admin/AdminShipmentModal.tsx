...
import { Shipment, ShipmentStatus, Role, NotificationCategory, Driver, ShipmentProduct, DeductionPrice } from '../../../types';
  ...
  return (
    <div className="bg-secondary-50 dark:bg-secondary-900 rounded-md overflow-hidden">
      <button
        ...
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <h4 className="font-bold text-lg">قسم الإستحقاقات</h4>
        <Icons.ChevronDown className={`h-5 w-5 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
      </button>
...
const AdminShipmentModal: React.FC<AdminShipmentModalProps> = ({ shipment, isOpen, onClose }) => {
  const {
    updateShipment, createInstallment, currentUser, addNotification, drivers, regions, regionConfigs,
  ...
  // Get region config to display road expenses in basic info
  const shipmentRegionConfig = regionConfigs
    .filter(rc => rc.regionId === shipment.regionId && rc.effectiveFrom <= shipment.orderDate)
    .sort((a, b) => b.effectiveFrom.localeCompare(a.effectiveFrom))[0];
  const regionRoadExpenses = currentShipment.roadExpenses ?? (shipmentRegionConfig?.roadExpenses || 0);
  ...
  const handleProductDeductionChange = (
    ...
    setCurrentShipment(prev => {
      const updatedProducts = prev.products.map(p => {
        ...
        const relevantPrices = deductionPrices.filter(dp =>
          dp.productId === productId && dp.effectiveFrom <= prev.orderDate
        );
  ...
  // Check if deductions were edited
  const hasDeductionsChanged = (): boolean => {
    // Check otherAmounts
    if ((currentShipment.otherAmounts ?? 0) !== (shipment.otherAmounts ?? 0)) return true;

    // Check products (shortage/damaged)
    for (const p1 of currentShipment.products) {
      const p2 = shipment.products.find(p => p.productId === p1.productId);
      if (!p2) return true;

      if (
        (p1.shortageCartons ?? 0) !== (p2.shortageCartons ?? 0) ||
        (p1.shortageExemptionRate ?? 0) !== (p2.shortageExemptionRate ?? 0) ||
        (p1.damagedCartons ?? 0) !== (p2.damagedCartons ?? 0) ||
        (p1.damagedExemptionRate ?? 0) !== (p2.damagedExemptionRate ?? 0)
      ) {
        return true;
      }
    }
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      let draftUpdates: Partial<Shipment> = { status: ShipmentStatus.DRAFT };

      if (hasDeductionsChanged() && currentUser) {
        draftUpdates.deductionsEditedBy = currentUser.username;
        draftUpdates.deductionsEditedAt = new Date().toISOString();
      }

      const draftShipment = { ...currentShipment, ...draftUpdates };
      ...
      if (err instanceof Error && (err.message.includes('NetworkError') || err.message.includes('CORS') || err.message.includes('fetch'))) {
        ...
        alert("فشل حفظ المسودة: " + ((err as Error).message || "خطأ غير معروف"));
      }
    } finally {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      let modifiedFields: Partial<Shipment> = {};
      ...
      if (hasDeductionsChanged() && currentUser) {
        modifiedFields.deductionsEditedBy = currentUser.username;
        modifiedFields.deductionsEditedAt = new Date().toISOString();
      }
      ...
      <div className="space-y-4 p-1">
        ...
        <ShipmentStepper
          ...
          dates={{
            ...
            accounting: (currentShipment.status === ShipmentStatus.SENT_TO_ADMIN || currentShipment.status === ShipmentStatus.DRAFT || currentShipment.status === ShipmentStatus.FINAL || currentShipment.status === ShipmentStatus.FINAL_MODIFIED) ? currentShipment.modifiedAt : undefined,
            admin: (currentShipment.status === ShipmentStatus.FINAL || currentShipment.status === ShipmentStatus.FINAL_MODIFIED) ? currentShipment.updated_at : undefined
          }}
        />
        ...
        <ItemizedDeductionsSection
          ...
          orderDate={currentShipment.orderDate}
